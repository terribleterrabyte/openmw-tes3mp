...
void ObjectList::reset()
{
    ...
    action = BaseObjectList::Action::Request;
    containerSubAction = BaseObjectList::ContainerSubAction::None;
}
...
void ObjectList::addEntireContainer(const MWWorld::Ptr& ptr)
{
    ...
    baseObjects.push_back(move(baseObject));
}
...
void ObjectList::editContainers(MWWorld::CellStore* cellStore)
{
    ...
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (ptrFound)
        {
            ...
            if (action == BaseObjectList::Action::Set)
                containerStore.clear();

            bool isLocalDrop = isLocalEvent && containerSubAction == BaseObjectList::ContainerSubAction::Drop;
            bool isLocalDrag = isLocalEvent && containerSubAction == BaseObjectList::ContainerSubAction::Drag;
            ...
            for (const auto &containerItem : baseObject.containerItems)
            {
                ...
                if (action == BaseObjectList::Action::Set || action == BaseObjectList::Action::Add)
                {
                ...
                else if (action == BaseObjectList::Action::Remove && containerItem.actionCount > 0)
                {
            ...
            if ((action == BaseObjectList::Action::Add || action == BaseObjectList::Action::Set) && hasActorEquipment &&
    ...
    for (const auto &baseObject : baseObjects)
    {
        ...
        if (!ptrFound)
        {
            try
            {
                ...
                if (baseObject.enchantmentCharge > -1.0f)
